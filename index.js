    ...
    try {
        ...
        let buttons = res.rows.map(r => {
            ...
            return [
                ...
                { text: '🗑️ Del', callback_data: `del:${r.id}` } 
            ];
        });
        ...
        return { text: '⚠️ Error loading reminders.', keyboard: { inline_keyboard: [[{ text: 'Error loading reminders', callback_data: 'noop' }]] } };
    }
}
    ...
    try {
        ...
        if (callbackQuery) {
            ...
            if (data.startsWith('settz:')) {
                ...
                await answerCallbackQuery(callbackQuery.id, 'Tap again to confirm deletion!', false);
                const dashData = await getRemindersDashboardData(userId, userTz, userFirstName); 
                ...
                await answerCallbackQuery(callbackQuery.id, 'Reminder deleted!', false);
                ...
                if (!iMsgId) {
                    ...
                    if (result.rows.length > 0) { 
                        const r = result.rows[0];
                        await editTelegramMessage(userId, callbackQuery.message.message_id, `Editing Reminder: "<b>${r.text}</b>"\nSelect options below:`, getEditMenuKeyboard(reminderId, r.recurring, r.total_occurrences, r.early_offset));
                    }
                    return res.sendStatus(200); 
                }
                ...
                if (iMsgId) {
                    ...
                    if (pendingInlineEdits.has(key)) {
                        ...
                        if (result.rows.length > 0) { 
                            ...
                            await sendOrUpdateDashboard(userId, `Editing Reminder: "<b>${r.text}</b>"\nSelect options below:`, getEditMenuKeyboard(reminderId, r.recurring, r.total_occurrences, r.early_offset));
                        }
                        ...
                        await answerCallbackQuery(callbackQuery.id, 'Tap Edit again within 10s to send options to your DM', false);
                        const dashData = await getRemindersDashboardData(userId, userTz, userFirstName); 
                            ...
                            dashData.keyboard.inline_keyboard = dashData.keyboard.inline_keyboard.map(row => { 
                                return row.map(btn => {
                                    if (btn.callback_data === `edit:${reminderId}`) {
                                        return { text: '🚀 Send DM?', callback_data: `edit:${reminderId}` };
                                    }
                                    return btn; 
                                });
                            });
                ...
                if (result.rows.length > 0) { 
                    const r = result.rows[0];
                    await editTelegramMessage(chatId, messageId, `Editing Reminder\nSelect options below:`, getEditMenuKeyboard(reminderId, r.recurring, r.total_occurrences, r.early_offset));
                }
                ...
                await editTelegramMessage(chatId, messageId, `<b>Please type the new note/text for this reminder:</b>`, { inline_keyboard: [[{ text: 'Cancel', callback_data: `edit:${reminderId}` }]] });
                await answerCallbackQuery(callbackQuery.id);
                ...
                await editTelegramMessage(chatId, messageId, `<b>Please type the new time/date for this reminder:</b>\n<i>Example: tomorrow at 8am, 2h, or Aug 12 5pm</i>`, { inline_keyboard: [[{ text: 'Cancel', callback_data: `edit:${reminderId}` }]] });
                await answerCallbackQuery(callbackQuery.id); 
                ...
                if (result.rows.length > 0) {
                    await answerCallbackQuery(callbackQuery.id);
                    await editTelegramMessage(chatId, messageId, `<b>Select specific days to repeat:</b>`, getDowMenuKeyboard(reminderId, result.rows[0].recurring));
                }
                ...
                if (result.rows.length > 0) {
                    ...
                    await answerCallbackQuery(callbackQuery.id);
                    await editTelegramMessage(chatId, messageId, `<b>Select specific days to repeat:</b>`, getDowMenuKeyboard(reminderId, newRec));
                }
            } else if (data.startsWith('unitmenu:')) {
                const reminderId = data.replace('unitmenu:', '');
                await answerCallbackQuery(callbackQuery.id);
                await editTelegramMessage(chatId, messageId, `<b>Select Custom Interval Unit:</b>`, getUnitMenuKeyboard(reminderId));

            } else if (data.startsWith('nummenu:')) { 
                const [, reminderId, unit] = data.split(':');
                await answerCallbackQuery(callbackQuery.id);
                await editTelegramMessage(chatId, messageId, `<b>Select Every How Many ${unit.toUpperCase()}:</b>`, getNumberMenuKeyboard(reminderId, unit));

            } else if (data.startsWith('prompt_rec:')) { 
                const [, reminderId, unit] = data.split(':');
                await setPendingEdit(userId, `rec:${reminderId}:${unit}`);
                await editTelegramMessage(chatId, messageId, `<b>Enter custom repeat interval in ${unit.toUpperCase()}:</b>\n<i>Example: 56, 72, 100</i>`, { inline_keyboard: [[{ text: 'Cancel', callback_data: `nummenu:${reminderId}:${unit}` }]] });
                await answerCallbackQuery(callbackQuery.id); 
                ...
                if (result.rows.length > 0) {
                    await answerCallbackQuery(callbackQuery.id);
                    await editTelegramMessage(chatId, messageId, `<b>Select How Many Times to Repeat:</b>`, getLimitMenuKeyboard(reminderId, result.rows[0].total_occurrences));  
                }
        ...
        if (inlineQuery) {
            ...
            if (queryText.toLowerCase() === 'view' || queryText === '') { 
                ...
                results.push({
                    ...
                    id: 'show_reminders_inline_v6',
                    title: 'View Active Reminders (Inline)',
                    description: 'Tap to view and manage your reminders right here.',
                    thumbnail_url: 'https://cdn.jsdelivr.net/gh/twitter/twemoji@14.0.2/assets/72x72/1f4dc.png', 
                    thumb_width: 72,
                    thumb_height: 72,
                    input_message_content: { message_text: 'Requesting active reminders list...' },
                    reply_markup: {
                ...
                if (!parsed) {
                    if (iMsgId) {
                        await fetch(`https://api.telegram.org/bot${TOKEN}/editMessageText`, {
                            ...
                            body: JSON.stringify({
                                inline_message_id: iMsgId,
                                text: '⚠️ I could not parse that reminder time. Please try again.',
                                parse_mode: 'HTML'
                            })
                        });
                    }
