...
bot.on("inline_query", async (ctx) => {
  const results = [
    ...
    {
      type: "article",
      id: "active_reminders_inline",
      title: "📋 View Active Reminders (Inline)",
      description: "Posts reminders in chat, collapses after 30s",
...
bot.on("chosen_inline_result", async (ctx) => {
  if (ctx.chosenInlineResult.result_id === "active_reminders_inline") {
...
`;
    if (rows.length === 0) {
      ...
      rows.forEach((r) => { text += `• ${r.text}
`; });
    }

    await ctx.telegram.editMessageText(null, null, inlineMessageId, text, { parse_mode: "Markdown" });

    setTimeout(async () => {
      try {
        await ctx.telegram.editMessageText(
          null, null, inlineMessageId, "📂 *Active Reminders (Collapsed)*",
          {
            parse_mode: "Markdown",
            reply_markup: { inline_keyboard: [[{ text: "Expand", callback_data: `expand_inline:${userId}` }]] }
          }
        );
      } catch (err) {
